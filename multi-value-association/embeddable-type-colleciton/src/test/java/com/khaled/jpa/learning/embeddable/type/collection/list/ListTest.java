package com.khaled.jpa.learning.embeddable.type.collection.list;

import com.khaled.jpa.learning.embeddable.type.collection.list.Adress;
import com.khaled.jpa.learning.embeddable.type.collection.list.Customer;
...
public class ListTest {
    ...
    static void generateDatabase() {
        emf = Persistence.createEntityManagerFactory("setupEmbeddableCollectionList");
        em = emf.createEntityManager();
    }

    @BeforeEach
    void createPersistenceContext() {
        emf = Persistence.createEntityManagerFactory("readyEmbeddableCollectionList");
        em = emf.createEntityManager();
    }
    ...
    @Test
    @Order(11)
    void duplicateAdressWithChange() {
        var paul = em.createQuery(
                "SELECT c FROM Customer c WHERE c.email = :email",
                Customer.class
        ).setParameter("email", PAUL.getEmail())
        ...
        em.getTransaction().begin();
        paul.addAdress(ALTERNATIVE_PAUL);
        ALTERNATIVE_PAUL.setCity("new city");
        em.getTransaction().commit();
        assertThat(paul.getAdresses().size(), is(3));
    }
}
